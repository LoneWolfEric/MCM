y_TX = xlsread('C:\Users\yuhui\Desktop\代码和论文\matlab\工作簿16.xlsx','E2:E34');
Data_TX=y_TX;              %共50个数据
SourceData_TX=Data_TX(1:23,1); %前250个训练集
step=10;                  %后50个测试
TempData=SourceData_TX;
TempData=detrend(TempData);%去趋势线
TrendData=SourceData_TX-TempData;%趋势函数
%--------差分，平稳化时间序列---------
H=adftest(TempData);
difftime=0;
SaveDiffData=[];
...
PreData=TrendResult+PreR(size(SourceData_TX',2)+1:size(PreR,2));
tempx_TX=[TrendData',TrendResult]+PreR;    % tempx为预测结果
plot(tempx_TX,'k');
hold on 
plot(Data_TX,'Color',[0.3 0.8 0.9]);
hold on
